...
asv=read.table("Input_files/RSSvsCBASS_ASV_table.txt", header = TRUE, row.names = 1)
...
all.l.1=all.l %>% filter(Experiment == "CB") %>% group_by( Time, Temperature, Genotype, Family) %>% summarise(Abundance=sum(Abundance))
all.l.2=all.l %>% filter(Experiment == "RSS") %>% group_by( Time, Temperature, Genotype,  Family) %>% summarise(Abundance=sum(Abundance))
